...
export default function ZaleceniaPage() {
  const { id } = useParams(); // id = konsultacji
  ...
  const handleSave = async () => {
    ...
    // 🔹 1. zapis w dokumencie konsultacji (dla prostego odczytu)
    await updateDoc(docRef, { zalecenia });

    // 🔹 2. dodatkowy zapis w kolekcji "zalecenia"
await addDoc(collection(db, "zalecenia"), {
  konsultacjaId: id,
  konId: konsultacja?.konId,      // zapisz referencję do konia
